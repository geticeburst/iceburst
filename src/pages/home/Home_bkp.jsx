...
import {
    ...
    LoadingOverlay, LoadingSpinner, LoadingText,
...
import {
    FiHome, FiUsers, FiSettings, FiX, FiSearch,
    FiUser, FiCircle, FiCloud, FiBox, FiServer,
    FiAlertTriangle,
    FiList,
    FiCopy,
    FiSliders,
...
import { getTimeSelectOption, getTimeRange, useLoadItems } from '../../utils/utils';
...
function Home() {
    ...
    const {
        ...
        queryFn: ({ queryKey, pageParam }) => getLogs({ ...queryKey[1], pageParam }),
        initialPageParam: 0,
        ...
        return Object.keys(schema).map((columnName) => {
            ...
            let columnConfig = {
                ...
                cell: (cell) => {
                    ...
                    const truncatedValue = value?.length > 100 ? value.slice(0, 200) + '...' : value;
            ...
            <Card>
                <CardBody>
                    <PropertyList>
                        <Property
                            label="TimeStamp"
                            value={<Text color="muted">{dayjs(data.original.timestamp).format()}</Text>}
                        />
                        <Property textTransform={'capitalize'} label="LogGroup" value={data.original.logGroup} />
                        {Object.keys(rawData).map((columnName, index) => {
                            return <Property key={index + 'id'}
                                label={<Box textTransform='capitalize' className=''>{columnName}</Box>}
                                value={<Box><Text flexWrap='nowrap' w='400px' color="muted" noOfLines={[1, 2, 3]}>{columnName.toLowerCase() == 'timestamp' ? dayjs(rawData[columnName]).format() : rawData[columnName]}</Text></Box>} />
                    ...
                    <Spacer />
                    <JsonView data={rawData} shouldExpandNode={allExpanded} style={darkStyles} />
                ...
                <PageHeader
                    ...
                    toolbar={
                        ...
                        <Toolbar>
                            <Flex align="center" justify="space-between" width="full">
                                ...
                                <Flex flex="1" justify="flex-end">
                                    ...
                                    >
                                        ...
                                        <ToggleButton value="dateRangePicker">
                                            Custom Range
                                        </ToggleButton>
                                    </ToggleButtonGroup>
                            ...
                            {/* <Box >
                                <SearchInput size="sm" width={{ base: 'full', lg: 60 }}
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    onReset={() => setSearchQuery('')}
                                />
                    ...
                    footer={<Box >
                        <Toolbar variant='outline' >
                            ...
                            >
                                <ToggleButton value="1" className={dateFilter == "1" && 'selected'} aria-selected={dateFilter == "1"}>Error </ToggleButton>
                                <ToggleButton value="2" className={dateFilter == "2" && 'selected'} aria-selected={dateFilter == "1"}>Live </ToggleButton>
                        ...
                        >
                            ...
                            <FiltersAddButton />
                            <Spacer />
                            {/* {!isLoading && <Menu closeOnSelect={false}>
                                ...
                                <MenuList zIndex={2}>
                                    ...
                                    >
                                        {dynamicColumns.map((c) => {
                                            ...
                                            return (
                                                <MenuItemOption value={c.accessorKey} key={c.accessorKey}>{c.accessorKey}</MenuItemOption>
                                            )
                                        })}
                                    </MenuOptionGroup>
                                </MenuList>
